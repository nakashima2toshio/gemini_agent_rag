...
RAG（Retrieval-Augmented Generation）における「ベクトル検索」の核心である、
ベクトルA（質問）とベクトルB（ドキュメント）の類似度の概念を図解と解説で示します。
RAGでは、文章の「意味」を数値の列（ベクトル）に変換し、空間上の「向き」の近さで類似度を判定します。

#### 1. ベクトル類似度の幾何学的イメージ

最も一般的に使われる「コサイン類似度（Cosine Similarity）」では、
ベクトルの長さ（文章の長さ）ではなく、**「角度（$\theta$）」の近さ（意味の方向）** が重要です。
...
```mermaid
flowchart LR
    subgraph Query_Side ["質問側"]
        Q["ユーザーの質問"] -->|"Embeddingモデル"| VA("ベクトル A")
    end

    subgraph Knowledge_Base ["知識ベース側"]
        D["ドキュメント"] -->|"Embeddingモデル"| VB("ベクトル B")
    end

    VA -->|"比較演算"| Sim{"類似度計算"}
    VB -->|"比較演算"| Sim

    Sim -->|"Cos類似度算出"| Score["スコア: 0.0 〜 1.0"]
  
    Score -- "スコアが高い" --> Match["検索結果として採用"]
    Score -- "スコアが低い" --> Ignore["無視"]
...
| 項目                       | Redis                                                                | Qdrant                                                               |
| :------------------------- | :------------------------------------------------------------------- | :------------------------------------------------------------------- |
| **主な用途**               | ・セッション管理<br>・キャッシュ<br>・一時データ保存                 | ・ベクトル検索（メイン）<br>・ナレッジベース<br>・長期データ保存     |
| **特徴**                   | ・超高速（インメモリ）<br>・TTL（有効期限）設定可能<br>・Pub/Sub機能 | ・ベクトル検索に特化<br>・フィルタリング機能が豊富<br>・永続化が堅牢 |
| **本プロジェクトでの役割** | ・会話履歴のキャッシュ<br>・セッション状態管理                       | ・RAGのベクトル検索<br>・ドキュメント保存                            |
